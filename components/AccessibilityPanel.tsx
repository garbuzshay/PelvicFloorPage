...
const AccessibilityPanel: React.FC<AccessibilityPanelProps> = ({
  ...
  const [hideImages, setHideImages] = useState(() => {
    return localStorage.getItem("hideImages") === "true";
  });
  const [saturation, setSaturation] = useState(100);
  const [focusedReading, setFocusedReading] = useState(() => {
    return localStorage.getItem("focusedReading") === "true";
  });
  ...
  // Toggle image visibility
  const toggleImages = () => {
    const newHideImages = !hideImages;
    setHideImages(newHideImages);
    localStorage.setItem("hideImages", newHideImages.toString());
    applyImageVisibility(newHideImages); // Apply immediately when toggled
  };

  // Function to apply the visibility of images based on hideImages state
  const applyImageVisibility = (hide: boolean) => {
    const images = document.querySelectorAll("img");
    images.forEach((img) => {
  ...
  const resetSettings = () => {
    ...
    localStorage.removeItem("focusedReading");
  ...
  // Add Focused Reading Overlay
  const applyFocusedReadingOverlay = () => {
    if (!document.getElementById("focusedReadingOverlay")) {
      const overlay = document.createElement("div");
      overlay.id = "focusedReadingOverlay";
      overlay.style.position = "fixed";
      overlay.style.top = "0";
  ...
  const moveFocusedReadingBar = (event: MouseEvent) => {
    ...
    if (overlay) {
      const mouseY = event.clientY;
      const height = 200; // Adjust this value to control the height of the spotlight
      overlay.style.clipPath = `polygon(0 0, 100% 0, 100% ${
        mouseY - height / 2
      }px, 0 ${mouseY - height / 2}px, 0 ${mouseY + height / 2}px, 100% ${
        mouseY + height / 2
      }px, 100% 100%, 0 100%)`;
    }
  };
  ...
  // Toggle Focused Reading Mode
  const toggleFocusedReading = () => {
    const newFocusedReading = !focusedReading;
    setFocusedReading(newFocusedReading);
    localStorage.setItem("focusedReading", newFocusedReading.toString());
    if (newFocusedReading) {
      applyFocusedReadingOverlay();
    } else {
      removeFocusedReadingOverlay();
    }
  ...
  useEffect(() => {
    ...
    if (hideImages) {
      applyImageVisibility(true); // Ensure images stay hidden on reload
    }
  }, []); // Only run on mount
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={toggleImages}
      >
        {hideImages ? "הצג תמונות" : "הסתר תמונות"}
      </button>
